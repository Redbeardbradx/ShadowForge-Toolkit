# src/shadowforge/modules/shield/proxy.py
import requests
...
def tor_session(timeout: int = 30) -> requests.Session:
    """
    Forge TOR session. Assumes TOR running on 9050 (control 9051). Ethical: Local anon tests only.
...
def anon_request(url: str, session: Optional[requests.Session] = None) -> Dict[str, Any]:
    """Veil a GET thru TOR. Returns JSON or error."""
    if session is None:
        session = tor_session()
    try:
        resp = session.get(url)
        return {"status": resp.status_code, "content": resp.json() if resp.headers.get('content-type') == 'application/json' else resp.text[:500]}  # Trunc tease
